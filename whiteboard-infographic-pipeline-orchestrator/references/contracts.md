...
- **Manual image handoff** owns conversion from preview-only model output into local PNG files under `images/`.
...
For local runs, `asset.kind` must be `file` before D/E. `inline_generation` is metadata only and is not consumable by D/E. `svg_preview` is not allowed as a substitute for the requested model-generated PNG in this pipeline.

## Image naming

The manual download target is:
...
The scripts validate PNG signature and dimensions. They cannot prove an image was generated by a model; the operator must confirm that manually in the handoff note.
