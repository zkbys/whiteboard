...
function findInputPaths(projectDir, args) {
  const boardDirs = [join(projectDir, "board"), join(projectDir, "assets", "board")];

  const voiceoverPath =
    resolvePath(projectDir, args.voiceover) ||
    firstExisting(projectDir, ["script/voiceover_segments.json", "voiceover_segments.json"]);

  const boardManifestPath =
    resolvePath(projectDir, args["board-manifest"]) ||
    findFile(boardDirs, (name) => name === "board_manifest.json" || name.endsWith(".board_manifest.json"));

  const motionPlanPath =
    resolvePath(projectDir, args["motion-plan"]) ||
    findFile(boardDirs, (name) => name === "motion_plan.json" || name.endsWith(".motion_plan.json"));

  return { voiceoverPath, boardManifestPath, motionPlanPath };
}

function resolveManifestImage(projectDir, manifestPath, manifest, explicitPath) {
  ...
  return (
    ...
    findFile([join(projectDir, "infographic", "images"), join(projectDir, "board"), join(projectDir, "assets", "board")], (name) =>
...
function validateInputs({ source, manifest, motionPlan, boardImagePath }) {
  ...
  if (!manifest?.canvas?.width || !manifest?.canvas?.height) {
    errors.push("board_manifest.json must contain canvas.width and canvas.height");
  }
  ...
  if (!motionPlan?.composition?.width || !motionPlan?.composition?.height) {
    errors.push("motion_plan.json must contain composition.width and composition.height");
  }
  ...
  const elements = new Map((manifest.elements || []).map((item) => [item.id, item]));
  for (const segment of motionPlan.segments || []) {
    ...
    if (segment.target && !elements.has(segment.target)) {
      errors.push(`motion_plan segment ${segment.id} target '${segment.target}' is missing from board_manifest elements[]`);
    }
    for (const action of segment.actions || []) {
      if (!SUPPORTED_ACTIONS.has(action.type)) {
        errors.push(`action ${segment.id}/${action.annotation || "unknown"} has unsupported type '${action.type}'`);
      }
      const element = elements.get(action.element);
      if (!element) {
        errors.push(`action ${segment.id}/${action.annotation || "unknown"} references missing element '${action.element}'`);
        continue;
      }
      if (!element.annotations || !element.annotations[action.annotation]) {
        errors.push(`action ${segment.id}/${action.annotation || "unknown"} is missing annotation on element '${action.element}'`);
      }
      if (!Number.isFinite(Number(action.duration)) || Number(action.duration) <= 0) {
        errors.push(`action ${segment.id}/${action.annotation || "unknown"} must have positive duration`);
      }
    }
  }
...
function loadExistingTiming(projectDir, audioDir) {
  const timingPath = firstExisting(projectDir, ["audio/voiceover_timing.json", "assets/audio/voiceover_timing.json"]);
  if (!timingPath) fail("--skip-tts requires audio/voiceover_timing.json or assets/audio/voiceover_timing.json");
  const timing = readJson(timingPath);

  const sourceAudio = firstExisting(projectDir, ["audio/narration.wav", "assets/audio/narration.wav"]);
  if (!sourceAudio) fail("--skip-tts requires audio/narration.wav or assets/audio/narration.wav");

  ensureDir(audioDir);
  const narrationPath = join(audioDir, "narration.wav");
  if (resolve(sourceAudio) !== resolve(narrationPath)) copyFileSync(sourceAudio, narrationPath);
  writeJson(join(audioDir, "voiceover_timing.json"), timing);
  ...
  return (
    motionActions.find(
      (item) =>
        ...
        (item.element === sourceAction.element && item.type === sourceAction.type && item.spokenAnchor === sourceAction.spokenAnchor),
    ) || motionActions[index]
  );
}
...
function buildActionTiming(action, segment, index, totalActions) {
  ...
  const segmentSpan = Math.max(0.12, Number(segment.end) - Number(segment.start));
  ...
  if (Number.isFinite(Number(action.anchorRatio))) {
    rawOffset = Number(segment.speechDuration || 0) * Number(action.anchorRatio);
  } else if (Number.isFinite(Number(action.offset))) {
    rawOffset = Number(action.offset);
  } else {
    rawOffset = (Number(segment.speechDuration || segmentSpan) * (index + 1)) / (totalActions + 1);
  }
...
function updateMotionPlan({ source, timing, motionPlan }) {
  ...
  const updatedSegments = (timing.segments || []).map((timedSegment) => {
    ...
    const baseActions = sourceActions.length > 0 ? sourceActions : motionActions;

    const actions = baseActions.map((baseAction, index) => {
      const motionAction = findMatchingAction(motionActions, baseAction, index) || {};
      return buildActionTiming({ ...motionAction, ...baseAction }, timedSegment, index, baseActions.length);
    });
    ...
    return {
      ...
      caption: timedSegment.caption || sourceSegment.caption || motionSegment.caption || sourceSegment.text,
      boardId: timedSegment.boardId || sourceSegment.boardId || motionSegment.boardId,
      target: timedSegment.target || sourceSegment.target || sourceSegment.targetElement || motionSegment.target,
      camera: timedSegment.camera || sourceSegment.camera || motionSegment.camera,
      actions,
    };
  });
  ...
  return {
    ...
    composition: {
      ...
      width: Number(motionPlan.composition?.width || source.composition?.width || 1920),
      height: Number(motionPlan.composition?.height || source.composition?.height || 1080),
...
function generateIndexHtml({ width, height, duration, boardWidth, boardHeight }) {
...
function createHyperframesProject({ hfDir, audioDir, manifest, motionPlan, boardImagePath, version }) {
  ...
  writeFileSync(join(hfDir, "index.html"), generateIndexHtml({ width, height, duration, boardWidth, boardHeight }));
...
async function main() {
  ...
  const { voiceoverPath, boardManifestPath, motionPlanPath } = findInputPaths(projectDir, args);
  if (!voiceoverPath || !existsSync(voiceoverPath)) fail("voiceover_segments.json not found");
  if (!boardManifestPath || !existsSync(boardManifestPath)) fail("board_manifest.json not found");
  if (!motionPlanPath || !existsSync(motionPlanPath)) fail("motion_plan.json not found");
  ...
  const boardImagePath = resolveManifestImage(projectDir, boardManifestPath, manifest, args["board-image"]);
  validateInputs({ source, manifest, motionPlan: originalMotionPlan, boardImagePath });
  ...
  if (dryRun) {
    console.log(JSON.stringify({ projectDir, voiceoverPath, boardManifestPath, motionPlanPath, boardImagePath, voice, valid: true }, null, 2));
    return;
  }
  ...
  const updatedMotionPlan = updateMotionPlan({ source, timing: audio.timing, motionPlan: originalMotionPlan });
  const backupPath = backupAndWriteMotionPlan(motionPlanPath, updatedMotionPlan);
  const hfProject = createHyperframesProject({ hfDir, audioDir, manifest, motionPlan: updatedMotionPlan, boardImagePath, version });
  ...
  if (!skipRender) {
    ...
    const delta = Math.abs(renderedDuration - Number(audio.timing.totalDuration));
    if (delta > 0.1) {
      fail(`Rendered MP4 duration mismatch: video=${renderedDuration.toFixed(3)}s timing=${Number(audio.timing.totalDuration).toFixed(3)}s`);
    }
  }
